...
<div class="sm:grid grid-cols-2 gap-20 w-4/5 mx-auto py-15 border-b border-gray-200 ">
        ...
        <div class="w-full">
            <h3 class='text-gray-700 font-bold text-4xl pb-4'><a href='{{$post->url($locale, $routeWithoutLocale)}}'>{{$post->title}}</a></h3>
            <h5 class=''>{{$post->subtitle}}</h5>
            <span class="text-gray-500">
                <span class="font-bold italic text-gray-800">By: </span> {{$post->post->author->name}} <span class="light-text">Posted at: </span> {{date('d M Y ', strtotime($post->post->posted_at))}}
            </span>
